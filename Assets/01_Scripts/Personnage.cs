...
public class Personnage : MonoBehaviour
{
    ...
    {
        if (gameObject.transform.position.x <= hit.point.x + 1.8 && gameObject.transform.position.x >= hit.point.x - 1.8 && gameObject.transform.position.z <= hit.point.z + 1.8 && gameObject.transform.position.z >= hit.point.z - 1.8) {
            isInAction = true;
            animator.SetBool("isArrived", true);
            animator.SetBool("goForward", false);
            animator.SetBool("goBackward", false);
